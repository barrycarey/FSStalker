...
def create_watch(new_watch: WatchSchema, token: Text, uowm: UnitOfWorkManager = Depends(get_uowm)):
    ...
    with uowm.start() as uow:
        ...
        if len(user.watches) >= user.patreon_tier.max_watches and not user.is_mod:
            raise HTTPException(status_code=422, detail='You have exceeded the max number of allowed watches')

        notification_svc_ids = [ns.id for ns in new_watch.notification_services]
        ...
        watch = Watch(
            ...
            owner=user,
            notification_services=[ns for ns in user.notification_services if ns.id in notification_svc_ids]
        )
        uow.watch.add(watch)
        uow.commit()
